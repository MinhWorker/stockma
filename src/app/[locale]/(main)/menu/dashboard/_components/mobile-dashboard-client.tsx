...
import { Package, TrendingUp, TrendingDown, CreditCard, Warehouse, ArrowLeftRight, ChevronDown, Check } from 'lucide-react';
...
export function MobileDashboardClient({ stats, inventories, selectedInventoryId }: Props) {
  ...
  const selectedName = selectedInventoryId
    ? inventories.find((i) => i.id === selectedInventoryId)?.name ?? t('allInventories')
    : t('allInventories');
  ...
  // ---------------------------------------------------------------------------
  // Secondary stat cards — 5 items, last one spans full width if count is odd
  // ---------------------------------------------------------------------------
  const secondaryCards = [
    <StatCard key="products"
      icon={Package} iconBg="bg-violet-500"
      label={t('stats.totalProducts')} value={String(stats.totalProducts)}
    ...
    />,
    <StatCard key="tx"
      icon={ArrowLeftRight} iconBg="bg-sky-500"
      label={t('stats.transactionsToday')} value={String(txToday)}
      sub={t('stats.recentCount', { count: stats.recentTransactions.length })}
    />,
    <StatCard key="revenue"
      icon={TrendingUp} iconBg="bg-blue-500"
      label={t('stats.actualRevenue')} value={formatPrice(stats.actualRevenue)}
    ...
    />,
    <StatCard key="debt"
      icon={CreditCard} iconBg="bg-rose-500"
      label={t('stats.openDebt')}
  ...
  const isOdd = secondaryCards.length % 2 !== 0;
  const gridCards = isOdd ? secondaryCards.slice(0, -1) : secondaryCards;
  const lastCard = isOdd ? secondaryCards[secondaryCards.length - 1] : null;
  ...
  return (
    <div className="space-y-6 pb-8">
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] text-muted-foreground leading-none mb-0.5">Kho hàng</p>
          ...
          <Drawer open={pickerOpen} onOpenChange={(v) => !v && setPickerOpen(false)}>
            <DrawerContent>
              <DrawerHeader className="pb-2">
                <DrawerTitle className="text-base">Chọn kho hàng</DrawerTitle>
              </DrawerHeader>
              <div className="px-4 pb-6 space-y-2">
                {/* All warehouses option */}
                <button
                  onClick={() => selectInventory(undefined)}
                  className={cn(
                ...
                >
                  <div className={cn('flex h-10 w-10 shrink-0 items-center justify-center rounded-xl',
                    !selectedInventoryId ? 'bg-primary/10' : 'bg-muted'
                  )}>
                    <Warehouse className={cn('h-5 w-5', !selectedInventoryId ? 'text-primary' : 'text-muted-foreground')} strokeWidth={1.75} />
                  </div>
                  <p className={cn('flex-1 text-sm font-medium', !selectedInventoryId && 'text-primary')}>
                    {t('allInventories')}
                  </p>
                  {!selectedInventoryId && <Check className="h-4 w-4 text-primary shrink-0" />}
                </button>
                ...
                {inventories.map((inv) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className={cn('flex-1 text-sm font-medium', isSelected && 'text-primary')}>
                        {inv.name}
                      </p>
      ...
      <section className="px-4 space-y-3">
        {/* Hero card — stock value, full width */}
        <div className="rounded-2xl bg-muted/40 border border-border/60 px-4 py-4 space-y-1.5">
          <div className="flex items-center gap-2">
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-emerald-500 shrink-0">
              <Warehouse className="h-3.5 w-3.5 text-white" strokeWidth={2} />
            </div>
            <p className="text-xs text-muted-foreground">{t('stats.stockValue')}</p>
          </div>
          <p className="text-2xl font-bold tabular-nums leading-tight">{formatPrice(stats.totalStockValue)}</p>
        </div>
      ...
      <section className="space-y-2">
        ...
        {stats.recentTransactions.length === 0 ? (
          ...
          <div className="mx-4 rounded-2xl border border-border bg-card overflow-hidden divide-y divide-border/60">
...
function StatCard({ icon: Icon, iconBg, label, value, sub, subColor, valueColor }: StatCardProps) {
  return (
    <div className="rounded-2xl bg-muted/40 border border-border/60 p-3.5 space-y-2">
      <div className="flex items-center gap-2">
